...
<html>
	<body>
		<custom:navigation father="System Settings" model="POI" addPath="system/poi-conf" addMethod="add" />
		<form name="queryResult" action="<%=basePath%>system/poi-conf!index" method="post">
			...
			<custom:table>
				...
				<custom:tableBody page="${page}" id="poiid" index="i" var="v">
					...
					<custom:tableItem>
						<custom:show path="system/poi-conf" id="${v.poiid}" />
						<custom:edit path="system/poi-conf" id="${v.poiid}" />
						<custom:del path="system/poi-conf" id="${v.poiid}" />
					</custom:tableItem>
				</custom:tableBody>
			</custom:table>
