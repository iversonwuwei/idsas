...
<head>
	...
	<body>
		<custom:navigation father="Profile" model="My Counselling List" operate="Add" saveMethod="sub2();" homePath="profile/mycounsell" />
		<form action="<%=basePath%>profile/mycounsell!create" name="queryResult" method="post">
			<input type="hidden" name="cs.isdelete" id="isdelete" value="F" /> 
			<div class="pop_main">
				<table cellspacing="0" cellpadding="0">
					<tr>
						...
						${cs.username}
						<input type="hidden" name="cs.userid" id="cs.userid" value="${cs.userid}" /> 
						<input type="hidden" name="cs.username" id="cs.username" value="${cs.username}" /> 
						</td>
					</tr>
					<tr>
						<td class="text_bg">Driver:</td>
						<td style="text-align: left;" class="valid">
						<s:select  list="driList" listKey="key" listValue="value" name="cs.driverid" id="cs.driverid" theme="simple" />
						<input type="hidden" name="cs.drivername" id="cs.drivername" value="${cs.drivername}" /> 
						</td>
					</tr>
					<tr>
