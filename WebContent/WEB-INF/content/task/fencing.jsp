...
$(function(){
	...
	<body onload="initMap();">
		...
		<custom:navigation father="Task Management" model="Geo Fencing" addPath="task/fencing"   addMethod="add" />
...
<div>
	...
	<div  id="basicMap" style="width:600px; height:390px;display: none;border:solid 1px #ccc;padding:10px;background:#eee; position-top:-10;"></div>
...
<form name="queryResult" id="feacn" action="<%=basePath%>task/fencing!edit" method="post">
...
<script type="text/javascript">
	...
	function rename1(){
		...
		var df=$("#df").val();
		document.forms['feacn'].action=basePath+'task/fencing!edit?id='+df+"&date="+new Date();
	...
	function createTree() {
		...
		$("#demo").jstree({
			...
			"json_data" : {
				"ajax" : {
					"url" : basePath + "task/fencing!getTree",
			...
			"search" : {
				...
				"ajax" : {
					"type": "POST",
					"url" : basePath+"task/fencing!search"
          		}
			}
		})
		/*绑定添加方法*/
...
//			}
			...
			// 将父节点ID和名称传到后台
			$.post(basePath + "task/fencing!add", {
		...
		.bind("rename.jstree", function(e, data) {
			// 将节点id和新输入的名称传到后台
			$.post(basePath + "task/fencing!rename", {
		...
		.bind("remove.jstree", function(e, data) {
			...
			$.post(basePath + "task/fencing!remove", {
				"id" : data.rslt.obj.attr("id")
			}, function(r) {
				if ("error" == r.status) {
					alert("Delete failed");
