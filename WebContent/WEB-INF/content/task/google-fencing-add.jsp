...
<script charset="UTF-8" type="text/javascript" src="<%=basePath%>scripts/specific/task/google-map.js"></script>
	...
	<custom:navigation father="Task Management" model="Geo Fencing" operate="Add" saveMethod="sub2();" homePath="task/google-fencing" />
	
	<form id="equip_form" action="<%=basePath%>task/google-fencing!create" target="" method="post"> 

	 <input type="hidden" size="300" name="fenname" id="fenname" />
	<body onload="initMap();">
		 
	<div class="pop_main">
	<table  border="0">  <tr><td rowspan="15" height="400px" width="700px">
	...
	<td><input type="text" name="caption" id="caption" style="width: 150px"></td>
	</tr>
	<tr  > <td class="text_bg" >Plate Number:</td><td>
	 <s:select id="vehicleid" name="vehicleid" list="list" listKey="vehicleid"     listValue="licenseplate" headerKey="-1" headerValue="Please choose" theme="simple"   cssStyle="width:150px;"  /> 
	...
	<tr>
	 ...
	 <tr height="300"><td colspan="2"></td>
	   ...
	   function catchMaterials(text) {
			var v = new Array();
			$.ajax({
				url :  basePath +"task/google-fencing!catchArch",
				type : "POST",
				async : false,
				data : {
					"vehname" : text
	      ...
	      function sub2(){
	    	  ...
	    	  $.ajax({
	  			...
	  			url : basePath + "task/google-fencing!check2",
	  			success : function(data) {
	  				 if('[false]' == data){
			  ...
			  if(null != listlength && (listlength>0 && listlength <= 3)){
