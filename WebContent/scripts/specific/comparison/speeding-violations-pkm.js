...
function doSearch() {
	...
	$.ajax({
		...
		url : basePath + "comparison/speeding-violations-pkm!getChartData",
	...
	$("#barChart").bind("plothover", function(event, pos, item) {	//鼠标over事件
		if (item) {
			if (previousPoint != item.dataIndex) {
				...
				showTooltip(item.pageX, item.pageY, 'Date: ' + tooltips[item.series.bars.order - 1] + '<br/>Speed ​​Range: ' + tickArr[item.dataIndex][1] + '<br/>Times Per KM: ' + y);
			}
		} else {
			$("#tooltip").remove();
			previousPoint = null;
