...
function doSearch() {
	...
	$.ajax({
		...
		url : basePath + "comparison/daily-speeding!getChartData",
		success : function(dataList) {
			...
			for (var i = 0, j = dataList.length; i < j; i++) {	//迭代生成图表数据
				...
				driverData[driverIndex][driverDataIndex - 1] = [tickAndValue[dataList[i].riqi], dataList[i].behSP];	//X轴通过tickAndValue关系对象取实际数值
			}
