...
import com.zdtx.ifms.specific.service.report.RedrirouManager;
...
public class RedrirouController extends ReportBase<List<String>> {
	/**
	 * 
	 */
	private static final long serialVersionUID = -8252866844117947664L;
	...
	@Autowired
	private RedrirouManager redMgr;
	...
	public String index() {
		...
		page = detMgr.getDrirouAll(page, dvo);
	...
	/*public String exportDetail(){
		try {
			this.index();
			String title  = "Report Driver By Route";
			Object[] obj = new Object[1];
			obj[0]=listDict;
			xlsFileName = disposeXlsName(title + DateUtil.formatDate(new Date()));
			xlsStream = detMgr.getRedrirouData(obj,page,flag,ges,les,dvo.getBehCont());
    	} catch (Exception e) {
			e.printStackTrace();
		}
		return "xls";
	...
	public String exportDetail() {
		try {
			this.index();
			String title = "Report_Driver_By_Route";
			xlsFileName = title + "_" + DateUtil.formatLongTimeDateWithoutSymbol(new Date()) + ".xlsx";
			super.xlsStream = redMgr.getRepDriverByRouteExcel();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "xlsx";
