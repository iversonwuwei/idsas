...
/**
 * @description 新实时定位数据websocket service端(Java 7)
 * @author Liu Jun
 * @since 2014年8月11日 下午3:45:55
 */
//@ServerEndpoint(value = "/websocket/realtime")S
//@WebServlet(name = "realtime", urlPatterns="/realtime", loadOnStartup = 8)
public class RealtimeWsServer extends HttpServlet {
	
	private static final long serialVersionUID = -3980135732390236818L;
	
	private static final Logger LOGGER = Logger.getLogger(RealtimeWsServer.class);

	@Autowired
	private SocketManager socketManager;
	
	private static final Set<Session> clients = new CopyOnWriteArraySet<Session>();
	...
	public void init() throws ServletException {	//加载方法
		...
		if (SocketUtil.SOCKET_GPS_ISOPEN.trim().equals("true")) {
			System.out.println("RealtimeWsServer7: start up automatic!");
			creatSocket();
		}
	}
	...
	public void onOpen(Session session) {
		...
		if (SocketUtil.SOCKET_GPS_ISOPEN.trim().equals("true")) {
			if(socket == null) {
				System.out.println("RealtimeWsServer7: start up manual!");
				creatSocket();
			}
		}
	}
	...
	class KeepAliveWatchDog implements Runnable {
		...
		public void run() {
			System.out.println("Start to keepAliveDelay7");
			while (running) {
	...
	class ReceiveWatchDog implements Runnable {
		public void run() {
			System.out.println("Start to ReceiveWatchDog7");
			while (running) {
				try {
					if (inputStream.available() > 0) {
	...
	private static void sendGPSData(String[] dataArray) {
		...
		for (Session session : clients) {
			try {
				...
				LOGGER.error("Failed to send GPS data to client: " + session.getId() + ", remove it!", e);
				clients.remove(session);
				try {
					session.close();
				} catch (IOException e1) {
	...
	private static void sendOffline(String[] dataArray) {
		...
		for (Session session : clients) {
			try {
				...
				LOGGER.error("Failed to send offline message to client: " + session.getId() + ", remove it!", e);
				clients.remove(session);
				try {
					session.close();
				} catch (IOException e1) {
	...
	private static void sendAlert(String[] dataArray) {
		...
		for (Session session : clients) {
			try {
				...
				LOGGER.error("Failed to send alert data to client: " + session.getId() + ", remove it!", e);
				clients.remove(session);
				try {
					session.close();
				} catch (IOException e1) {
