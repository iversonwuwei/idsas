...
/**
 * 
 * @ClassName: SpeedingViolationsPkmManager
 * @Description: comparison-Speeding Violations Per KM-manager
 * @author: Leon Liu
 * @date: 2013-9-27 13:46:29
 * @version V1.0
 */
@Service
@Transactional
public class SpeedingViolationsPkmManager {
	...
	public List<SpeedingViolationsVO> getBatch(ChartVO vo) {
		...
		String sql = "SELECT WEEKLY AS WEEK," +
								" SUM(SP1)/SUM(RUNKM) AS SP1," +
								" SUM(SP2)/SUM(RUNKM) AS SP2," +
								" SUM(SP3)/SUM(RUNKM) AS SP3," +
								" MAX(SPV) AS SPV" +
