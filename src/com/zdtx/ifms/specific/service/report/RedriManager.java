...
public class RedriManager {
	...
	public InputStream getReqDriverExcel() {
		...
		Object[] counts = (Object[]) Utils.getSession().getAttribute("redricount");
		final String dateType = vo.getBehCont();
		DecimalFormat format = new DecimalFormat("####.##");
		
		OPCPackage opcPackage = null;
		XSSFWorkbook workbook = null;
		InputStream fis = this.getClass().getResourceAsStream("/../../excel-temp/temp_reportDriver.xlsx");
		...
		try {
			...
			row.getCell(4).setCellValue(vo.getDriverName());	//set department
			...
			for(int i=0;i<counts.length;i++){
				...
				row.getCell(i + 5).setCellValue(Utils.isEmpty(counts[i]) ? 0 : Double.valueOf(counts[i]+""));
			}
			//first row
			row = sheet.getRow(4);
			...
			for (int i = 0; i < violationCount; i++) {	//add violations data
				row.createCell(i + 7).setCellValue(Utils.isEmpty(obj[i + 7]) ? 0 : Double.valueOf(obj[i +7] + ""));
			}
			...
			for (int i = 1; i < chartData.size(); i++) {	
				...
				row.createCell(6).setCellValue(Double.valueOf(format.format(score)));
				for (int j = 0; j < violationCount; j++) {	//add violations data
					row.createCell(j + 7).setCellValue(Utils.isEmpty(obj[j + 7]) ? 0 : Double.valueOf(obj[j + 7] + ""));
				}
				if(score < 50) {	//all row's cell  red font when score <= 50
					for (int j = 0, k = row.getLastCellNum(); j < k; j++) {
						row.getCell(j).setCellStyle(redCellStyle);
			...
			CTPieSer ser = chart.getCTChart().getPlotArea().getPieChartList().get(0).getSerArray(0);	//get pie object
			String charSeq = "IJKLMNOPQRST";	//excel column's sequence
			for (int i = 0; i < violationCount - 1; i++) {
				ser.getCat().getStrRef().setF("'Report Driver'!$H$3:$" + charSeq.charAt(i) + "$3");
				ser.getVal().getNumRef().setF("'Report Driver'!$H$4:$" + charSeq.charAt(i) + "$4");
			}
			workbook.write(baos);
