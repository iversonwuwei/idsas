...
public class RedridepManager {
	...
	public InputStream getRepDriverByDeptExcel() {
		...
		InputStream fis = this.getClass().getResourceAsStream("/../../excel-temp/temp_reportDriverByDepartment.xlsx");
		...
		try {
			...
			row.getCell(1).setCellValue(obj[2] + "");
			row.getCell(2).setCellValue(obj[1] + "");
			...
			for (int i = 1; i < chartData.size(); i++) {	
				...
				row.createCell(1).setCellValue(obj[2] + "");	//Department column
				row.createCell(2).setCellValue(obj[1] + "");	//Driver column
			...
			ser.getCat().getNumRef().setF("'Report Driver By Department'!$C$4:$C$" + (chartData.size() + 3));
			ser.getVal().getNumRef().setF("'Report Driver By Department'!$G$4:$G$" + (chartData.size() + 3));
			workbook.write(baos);
